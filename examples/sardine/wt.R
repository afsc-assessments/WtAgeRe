...
do_fit <- function(fn="pnw.dat",fitmod=TRUE,source="PNW"){
  df <-read_dat(fn)
  #df_tmp <- data.frame(year=df$fshry_yrs,df$fishery)
  #names(df_tmp) <- c("year",0:10)
...
df_res <- fn_get_pred(file='wt',source="Sardine Mex1")
df_mex1  <- do_fit(fn="mex1.dat",source="Mex1")
df_mex2  <- do_fit(fn="mex2.dat",source="Mex2")
fn_plot_anoms(df_res)
df_res <- fn_get_pred(file='wt',source="Sardine Mex2")

df_fn<-do_fit(fn="PNW",source="PNW")
...
df$fshry_yrs
names(df)
df_fsh <- data.frame(year=df$fshry_yrs[,1:2],df$fishery[,1:2])
names(df_fsh) <- c("year",0:10)
